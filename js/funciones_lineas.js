$('#btnagregarlinea1').click(function(){
	Numero=$('#modal-numeroProduccion').val();
	Descripcion=$('#modal-descripcion').val();
	id_supervisor=$('#id_supervisor').val();
	cadena='Numero='+Numero+'&Descripcion='+Descripcion+'&id_supervisor='+id_supervisor;
$.ajax({
	type:"POST",
	url:"../adm/funciones_lineas.php?accion=agregarlinea1",
	data:cadena,
	success:function(r){
		//alert(r);
	if(r==1){
		window.location='linea1.php';
	}
	else{
		alert('No se pudo ingresar los datos');	
	}
	}
});
});

$('#btnagregarlinea2').click(function(){
	Numero=$('#modal-numeroProduccion').val();
	Descripcion=$('#modal-descripcion').val();
	id_supervisor=$('#id_supervisor').val();
	cadena='Numero='+Numero+'&Descripcion='+Descripcion+'&id_supervisor='+id_supervisor;
$.ajax({
	type:"POST",
	url:"../adm/funciones_lineas.php?accion=agregarlinea2",
	data:cadena,
	success:function(r){
		//alert(r);
	if(r==1){
		window.location='linea2.php';
	}
	else{
		alert('No se pudo ingresar los datos');	
	}
	}
});
});

$('#btnagregarlinea3').click(function(){
	Numero=$('#modal-numeroProduccion').val();
	Descripcion=$('#modal-descripcion').val();
	id_supervisor=$('#id_supervisor').val();
	cadena='Numero='+Numero+'&Descripcion='+Descripcion+'&id_supervisor='+id_supervisor;
$.ajax({
	type:"POST",
	url:"../adm/funciones_lineas.php?accion=agregarlinea3",
	data:cadena,
	success:function(r){
		//alert(r);
	if(r==1){
		window.location='linea3.php';
	}
	else{
		alert('No se pudo ingresar los datos');	
	}
	}
});
});
